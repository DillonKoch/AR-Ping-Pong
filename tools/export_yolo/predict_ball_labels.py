#!/usr/bin/env python3
"""Export ball detection predictions for the browser labeler."""
...
def main() -> None:
    ...
    while True:
        ...
        if frame_index % args.stride == 0:
            ...
            ball = best_ball_prediction(result)
            if ball:
                predictions.append(
                    {
                        "frame": frame_index,
    ...
    print(f"wrote {len(predictions)} ball predictions to {args.out}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=Path, required=True, help="Path to YOLO detection .pt file")
...
def best_ball_prediction(result: Any) -> dict[str, Any] | None:
